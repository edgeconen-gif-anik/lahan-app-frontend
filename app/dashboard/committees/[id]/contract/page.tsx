...
type CommitteeContractFormData = {
  ...
  officeGrant: number | string;
  communityContribution: number | string;
  beneficiaryHouseholds: number | string;
  beneficiaryPopulation: number | string;
  startDate: string;
  endDate: string;
};
...
export default function CommitteeContractPage() {
  ...
  const [formData, setFormData] = useState<CommitteeContractFormData>({
    projectId: "",
    userCommitteeId: "",
    contractNumber: "",
  ...
  const committeesList = (
    Array.isArray(committeesData) ? committeesData : committeesData?.data || []
  ) as CommitteeOption[];

  const handleChange = (
    event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>,
  ) => {
    const field = event.target.name as keyof CommitteeContractFormData;
  ...
  const calculateContribution = () => {
    ...
    if (total > 0) {
      const communityShare = total * 0.1; // Mandatory 10%
      setFormData({
        ...formData,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createContract.mutate({
      ...
      userCommitteeId: formData.userCommitteeId,
      ...
      intendedCompletionDate: formData.endDate,
    });
  };
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-3xl font-bold">
          Committee Agreement (योजना सम्झौता फाराम)
        </h1>
      </div>
      ...
      >
        ...
        <h3 className="text-lg font-bold border-b pb-2">
          Cost Details (लागत सम्वन्धि विवरण)
        </h3>
          ...
          <div className="space-y-2 relative">
            ...
            <div className="flex gap-2">
              <input
                ...
                readOnly
                className="w-full p-2 border border-green-300 rounded-md bg-green-50"
              />
              <button
                type="button"
                onClick={calculateContribution}
                className="p-2 bg-green-600 text-white rounded-md hover:bg-green-700"
                title="Auto Calculate 10%"
              >
                <Calculator size={20} />
              </button>
            </div>
